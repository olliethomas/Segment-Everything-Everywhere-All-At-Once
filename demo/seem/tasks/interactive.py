...
def interactive_infer_video(model, audio_model, image, tasks, refimg=None, reftxt=None, audio_pth=None, video_pth=None):
    ...
    output_file = video_pth.replace('.mp4', f'_{random_number}_output.mp4')
    frame_interval = 10
    ...
    ffmpeg_cmd = f'ffmpeg -i {video_pth} -vf \"fps=5\" {input_dir}/%04d.png'
    ...
    ffmpeg_cmd = f"ffmpeg -framerate 5 -pattern_type glob -i '{output_dir}/*.png' -c:v libx264  {output_file}"
    os.system(ffmpeg_cmd)
